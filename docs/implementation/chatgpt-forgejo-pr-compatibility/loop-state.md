...
S5 pass `1/3` head `e518223132fb2f0ca887f84398fce84d10cd2127` passed the full locked local gate and exact-head pull-request run `32842071549` plus push run `32842067065`. Independent review found one required typed-slot escape: `--watch` or `--help` can occupy the nonempty PR-number wildcard and still trigger capture. Pass-2 red head `e1dfe16659675a00f4d9e51366ba420b4fca5462` freezes both exact repros. The repair derives one typed capture/deadline policy from `ParsedInvocation`; the policy moves to `src/observation.rs` so routing stays below the 1,000-line review boundary and trace stays serialization-only. The coordinator remains the sole writer and PR #29 remains the sole external integration PR.
