...
Rust behavior commands should fail clearly until their owning phases port them. The phase-02 Rust test harness loads the phase-01 compatibility contract directly and runs binaries under temporary homes, temporary repositories, and fake environment maps so later phases can add parity checks without touching user configuration.
